...
class GUI(Frame):
    ...
    def centrar(self):
        ...
        posicion = str(ancho) + "x" + str(alto) + "+" + \
            str(x_ventana) + "+" + str(y_ventana - 35)
    ...
    def ventanaFunciones(self):
        ...
        v2.protocol("WM_DELETE_WINDOW",
                    lambda: self.on_closing(v2, self.master))
    ...
    def ventanaReporte(self):
        ...
        v3.protocol("WM_DELETE_WINDOW",
                    lambda: self.on_closing(v3, self.master))
    ...
    def funciones(self):
        lbl1 = Label(self.master, text="Bases de datos", font=(
            "Century Gothic", 21), bg="#0f1319", fg="#ffffff")
        lbl1.grid(row=1, column=0, padx=(60, 85), pady=(100, 25))

        lbl2 = Label(self.master, text="Tablas", font=(
            "Century Gothic", 21), bg="#0f1319", fg="#ffffff")
        lbl2.grid(row=1, column=1, padx=(5, 0), columnspan=2, pady=(100, 25))

        lbl3 = Label(self.master, text="Tuplas", font=(
            "Century Gothic", 21), bg="#0f1319", fg="#ffffff")
        ...
        btnExtractRangeTb = Button(self.master,
                                   ...
                                   command=lambda: self.simpleDialog(["database", "table", "column", "lower", "upper"],
                                                                     self.actions[8]))
        btnExtractRangeTb.grid(row=6, column=2, sticky=W,
                               padx=(5, 0), pady=(0, 25))
        # endregion
        ...
        btnInsertTp.grid(row=2, column=3, sticky=W,
                         padx=(110, 0), pady=(0, 25))
        ...
        btnLoadfile.grid(row=3, column=3, sticky=W,
                         padx=(110, 0), pady=(0, 25))
        ...
        btnExtractTp.grid(row=4, column=3, sticky=W,
                          padx=(110, 0), pady=(0, 25))
        ...
        btnUpdateTp = Button(self.master,
                             ...
                             command=lambda: self.simpleDialog(["database", "table", "register", "columns"],
                                                               self.actions[20]))
        btnUpdateTp.grid(row=5, column=3, sticky=W,
                         padx=(110, 0), pady=(0, 25))
        ...
        btnDeleteTp.grid(row=6, column=3, sticky=W,
                         padx=(110, 0), pady=(0, 25))
        ...
        btnTruncateTp.grid(row=7, column=3, sticky=W,
                           padx=(110, 0), pady=(0, 25))
        # endregion
    ...
    def reportes(self):
        ...
        self.tableList = Combobox(
            self.master, state="readonly", width=30, font=("Century Gothic", 12))
        ...
        self.canvas = Canvas(self.master, width=740,
                             height=415, bg="#0f1319", highlightthickness=0)
    ...
    def desplegarDB(self):
        try:
            ...
            self.image_on_canvas = self.canvas.create_image(
                370, 207, anchor=CENTER, image=bg_DB)
            # self.canvas.config(xscrollcommand=self.scrollbarX.set, yscrollcommand=self.scrollbarY.set)
    ...
    def displayTupleData(self, event):
        try:
            ...
            # le envío el índice del árbol (tp)
            png = self.controller.reportTPL(db, tb, tp)
    ...
    def simpleDialog(self, args, action):
        ...
        if args[0] == "file":
            btnFile = Button(dialog, text="Examinar...",
                             command=lambda: self.cargarArchivo(btnFile))
            btnFile.grid(row=0, column=1, pady=(
                15, 2), padx=(0, 18), sticky="ew")
            ...
            for j in range(dim):
                ...
                tmp.append(entry)
        tmp[0].focus_set()
        dialog.bind('<Return>', lambda event=None: self.ejecutar(
            dialog, tmp, action))
