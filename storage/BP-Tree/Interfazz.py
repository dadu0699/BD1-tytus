...
def show_data_bases():
    ...
    canvas_tree.bind('<Configure>', lambda e: canvas_tree.configure(
        scrollregion=canvas_buttons.bbox('all')))
    scroll_tree = Scrollbar(main_tree, orient=HORIZONTAL,
    ...
    canvas_tree.create_window(
        (0, 0), width=10000, height=5000, window=frame_tree, anchor='nw')
    ...
    canvas_buttons = Canvas(
        tree_window, background='red', width=130, height=470)
    canvas_buttons.place(x=25, y=75)
    scroll_buttons = Scrollbar(
        main_tree, orient=VERTICAL, command=canvas_buttons.yview)
    scroll_buttons.pack(side=LEFT, fill=Y)
    canvas_buttons.configure(yscrollcommand=scroll_buttons.set)
    canvas_buttons.bind('<Configure>', lambda e: canvas_buttons.configure(
        scrollregion=canvas_buttons.bbox('all')))
    buttons_frame = Frame(canvas_buttons)
    canvas_buttons.create_window(
        (15, 0), width=130, height=5000, window=buttons_frame, anchor='nw')
    yview = 5
    names = []
    for x in data_bases:
        names.append(x)
        Button(buttons_frame, text=x, font='Helvetica 8 bold italic', fg='white', bg='black',
               command=lambda database=x: show_tables(tree_window, database), width=15, padx=15, pady=5).place(x=0, y=yview)
        yview += 35
    Button(main_tree, text='Regresar', font='Helvetica 10 bold italic', command=lambda: close_table_window(
        tree_window, main_window), bg='red', width=15, pady=3).place(x=25, y=15)
    Button(main_tree, text='Create', font='Helvetica 10 bold italic', bg='white', padx=20,
           pady=3, command=lambda: create_database_window(tree_window)).place(x=170, y=15)
    Button(main_tree, text='Alter', font='Helvetica 10 bold italic', bg='white', padx=20,
           pady=3, command=lambda: alter_database_window(tree_window, names)).place(x=270, y=15)
    Button(main_tree, text='Drop', font='Helvetica 10 bold italic', bg='white', padx=20,
           pady=3, command=lambda: drop_database_window(tree_window, names)).place(x=370, y=15)
    Button(main_tree, text='Show Tables', font='Helvetica 10 bold italic', bg='white', padx=20,
           pady=3, command=lambda: show_tables_window(tree_window, names)).place(x=470, y=15)
...
def show_tables(parent_window, database):
    ...
    canvas_tree.create_window(
        (0, 0), width=10000, height=5000, window=frame_tree, anchor='nw')
    canvas_tree.image = PhotoImage(
        file=f'./Data/BPlusMode/{database}/{database}.png')
    ...
    buttons_frame = Frame(canvas_buttons)
    canvas_buttons.create_window(
        (15, 0), width=120, height=6000, window=buttons_frame, anchor='nw')
    yview = 5
    names = []
    for x in tables:
        names.append(x)
        Button(buttons_frame, text=x, font='Helvetica 8 bold italic', fg='white', bg='black', command=lambda table=x: extract_table(
            database, table, table_window), width=15, padx=15, pady=5).place(x=0, y=yview)
        yview += 35
...
def extract_table(database, table, parent_window):
    ...
    canvas_tree.create_window(
        (0, 0), width=10000, height=5000, window=frame_tree, anchor='nw')
    canvas_tree.image = PhotoImage(
        file=f'./Data/BPlusMode/{database}/{table}/{table}.png')
    Label(frame_tree, image=canvas_tree.image).place(x=50, y=50)
    ...
    buttons_frame = Frame(canvas_buttons)
    canvas_buttons.create_window(
        (15, 0), width=120, height=5000, window=buttons_frame, anchor='nw')
    Button(table_window, text='Regresar', font='Helvetica 10 bold italic', command=lambda: close_table_window(
        table_window, parent_window), bg='red', width=15, pady=1.5).place(x=25, y=15)
    yview = 5
    names = []
    for x in range(0, len(list(keys))):
        Button(buttons_frame, text=keys[x], font='Helvetica 8 bold italic',  fg='white', bg='black', command=lambda info=rows[x], key=keys[x]: table_graph(
            info, key, table, database), width=15, padx=15, pady=5).place(x=0, y=yview)
        yview += 35
    ...
    tkinter.Label(table_window, bg='yellow', text='Columnas: '+getColumn(database, table)
                  [0], font='Helvetica 10 bold italic', width=10, padx=20, pady=3).place(x=25, y=50)
    tkinter.Label(table_window, bg='yellow', text='PK: '+",".join(str(x) for x in getColumn(database,
                                                                                            table)[1]), font='Helvetica 10 bold italic', width=10, padx=20, pady=3).place(x=25, y=80)
...
def table_graph(tupla, key, table, database):
    f = open(
        f'Data/BPlusMode/{database}/{table}/tupla.dot', 'w', encoding='utf-8')
    ...
    f.write('table [label = ' + tabla +
            ',  fontsize="30", shape = plaintext ];\n')
    f.write('}')
    f.close()
    Storage.os.system(
        f'dot -Tpng Data/BPlusMode/{database}/{table}/tupla.dot -o ./Data/BPlusMode/{database}/{table}/tupla.png')
    ...
    scroll = Scrollbar(info_window, orient=HORIZONTAL,
                       command=tupla_canvas.xview)
    scroll.pack(side=BOTTOM, fill=X)
    tupla_canvas.configure(xscrollcommand=scroll.set)
    tupla_canvas.bind('<Configure>', lambda e: tupla_canvas.configure(
        scrollregion=tupla_canvas.bbox('all')))
    photo_frame = Frame(tupla_canvas)
    tupla_canvas.create_window(
        (0, 0), width=3000, height=300, window=photo_frame, anchor='nw')
    tupla_canvas.image = PhotoImage(
        file=f'./Data/BPlusMode/{database}/{table}/tupla.png')
...
def create_database_window(parent):
    ...
    tkinter.Label(create_window, text='Create Database',
                  font='Helvetica 10 bold italic', width=20).place(x=5, y=70)
    database_name = Entry(create_window, width=25)
    database_name.place(x=140, y=70)
    tkinter.Button(create_window, text='Create', font='Helvetica 10 bold italic', width=10, command=lambda: create_database(
        database_name.get(), database_name, create_window, parent)).place(x=300, y=65)
...
def create_database(database, database_name, window, parent):
    if database:
        ...
        if result == 0:
            messagebox.showinfo(title='Create Database',
            ...
            messagebox.showinfo(title='Create Database',
                                message='Base de datos existente')
            database_name.delete(0, END)
    else:
        messagebox.showinfo(title='Create Database',
                            message='No lleno el campo de texto')
...
def alter_database_window(parent, names):
    if names:
        ...
        tkinter.Label(window, text='Alter Database',
                      font='Helvetica 10 bold italic', width=20).place(x=25, y=70)
        box = OptionMenu(window, variable, *names)
        box.place(x=165, y=65)
        Label(window, text='New Name', font='Helvetica 10 bold italic',
              width=20).place(x=250, y=50)
...
def alter_database(old, new, entry1, window, parent):
    if old and new:
        ...
        if result == 0:
            messagebox.showinfo(title='Alter Database',
                                message='Operacion exitosa')
            window.destroy()
            parent.destroy()
            show_data_bases()
            ...
            messagebox.showinfo(title='Alter Database',
                                message=old + ' no existe')
            entry1.delete(0, END)
        elif result == 3:
            messagebox.showinfo(title='Alter Database',
                                message=new + ' ya existe')
            entry1.delete(0, END)
    else:
        messagebox.showinfo(title='Alter Database',
                            message='No lleno los campos de texto')
        entry1.delete(0, END)
...
def drop_database_window(parent, names):
    if names:
        ...
        variable = StringVar(window)
        variable.set(names[0])
        tkinter.Label(window, text='Drop Database',
                      font='Helvetica 10 bold italic', width=20).place(x=25, y=70)
        box = OptionMenu(window, variable, *names)
        box.place(x=165, y=65)
...
def drop_database(database, window, parent):
    if database:
        ...
        if result == 2:
            messagebox.showinfo(title='Drop Database',
...
def create_table_window(grandParent, parent, database):
    ...
    Label(window, text='Create Table',
          font='Helvetica 10 bold italic', width=20).place(x=50, y=70)
    Label(window, text='Name', font='Helvetica 10 bold italic',
          width=20).place(x=130, y=50)
    Label(window, text='Columns', font='Helvetica 10 bold italic',
          width=10).place(x=260, y=50)
...
def create_data_table(database, table, columns, entry1, entry2, window, parent, grandParent):
    if database and table and columns:
        if isNumber(columns):
            ...
            if result == 3:
                messagebox.showinfo(title='Create table',
                                    message=table + ' ya existe')
                entry1.delete(0, END)
                entry2.delete(0, END)
            elif result == 2:
                messagebox.showinfo(title='Create table',
            ...
            messagebox.showinfo(title='Create table',
                                message="No escribio un numero de columnas")
            entry1.delete(0, END)
    else:
        messagebox.showinfo(title='Create table',
                            message='No lleno el campo de texto')
...
def function_show_tables(database, window, parent):
    if database:
        messagebox.showinfo(
            title='Show tables', message=f'{database}: {Storage.showTables(database)}')
...
def extract_table_function(database, table):
    if database and table:
        ...
        if result is not None:
            messagebox.showinfo(title='Extract table',
                                message='Operacion exitosa: ' + str(result))
        else:
            messagebox.showinfo(
                title='Extract table', message='Base de datos o tabla no exsitentes: ' + str(result))
    else:
        messagebox.showinfo(title='Create Database',
                            message='No se llenaron los campos de texto')
...
def extract_range_table_window(database, names):
    if names:
        ...
        Label(window, text='Extract Range',
              font='Helvetica 10 bold italic', width=15).place(x=10, y=70)
        Label(window, text='Column', font='Helvetica 10 bold italic',
              width=10).place(x=200, y=50)
        Label(window, text='Lower', font='Helvetica 10 bold italic',
              width=10).place(x=270, y=50)
        Label(window, text='Upper', font='Helvetica 10 bold italic',
              width=10).place(x=340, y=50)
...
def extract_range_table(database, table, columnNumber, lower, upper, entry1, entry2, entry3):
    if database and table and columnNumber and lower and upper:
        if isNumber(columnNumber):
            result = Storage.extractRangeTable(
                database, table, int(columnNumber), lower, upper)
            if result is not None:
                messagebox.showinfo(title='Extract range table',
                                    message='Extract completado: ' + str(result))
                entry1.delete(0, END)
                entry2.delete(0, END)
                entry3.delete(0, END)
            else:
                messagebox.showinfo(title='Extract range table',
                                    message='Datos no existentes: ' + str(result))
                entry1.delete(0, END)
                entry2.delete(0, END)
                entry3.delete(0, END)
        else:
...
def alter_table_window(grandParent, parent, database, names):
    if names:
        ...
        variable = StringVar(window)
        variable.set(names[0])
        tkinter.Label(window, text='Alter Table',
                      font='Helvetica 10 bold italic', width=20).place(x=25, y=70)
        box = OptionMenu(window, variable, *names)
        box.place(x=165, y=65)
        Label(window, text='New Name', font='Helvetica 10 bold italic',
...
def drop_table_window(grandParent, parent, database, names):
    if names:
        ...
        variable = StringVar(window)
        variable.set(names[0])
        tkinter.Label(window, text='Drop Table',
                      font='Helvetica 10 bold italic', width=20).place(x=25, y=70)
        box = OptionMenu(window, variable, *names)
        box.place(x=165, y=65)
        tkinter.Button(window, text='Drop', font='Helvetica 10 bold italic', width=10, command=lambda:
...
def drop_table(grandParent, parent, actual, database, table):
    if database and table:
        ...
        if result == 0:
            messagebox.showinfo(title='Alter DropPK',
                                message='Operacion exitosa')
            actual.destroy()
            refresh_tables(grandParent, parent, database)
        elif result == 1:
            ...
            messagebox.showinfo(title='Alter DropPK',
                                message='Base de datos no existente')
        elif result == 3:
            messagebox.showinfo(title='Alter DropPK',
                                message='Tabla no existente')
    else:
        messagebox.showinfo(title='Alter DropPK',
                            message='No se llenaron los campos de texto')
# --------ROWS----------------
...
def alter_addPK_window(grandParent, parent, database, table):
    ...
    window.geometry('500x200+200+100')
    window.title('Add PK')
    tkinter.Label(window, text='Add Pk',
                  font='Helvetica 10 bold italic', width=20).place(x=25, y=70)
    Label(window, text='PK', font='Helvetica 10 bold italic',
          width=20).place(x=140, y=50)
...
def alter_addPK(grandParent, parent, actual, database, table, columns, entry1):
    if database and table and columns:
        ...
        for x in separate:
            if not isNumber(x):
                messagebox.showinfo(
                    title='Alter AddPK', message='No ingreso algun numero de columna')
                entry1.delete(0, END)
                return
            pk.append(int(x))
        result = Storage.alterAddPK(database, table, pk)
        if result == 0:
            messagebox.showinfo(title='Alter AddPK',
                                message='Operacion exitosa')
            actual.destroy()
            ...
            messagebox.showinfo(title='Alter AddPK',
                                message='Llave primaria existente')
            entry1.delete(0, END)
        elif result == 5:
            messagebox.showinfo(title='Alter AddPK',
                                message='Columnas fuera de limites')
            entry1.delete(0, END)
    else:
        messagebox.showinfo(title='Alter AddPK',
                            message='No se llenaron los campos de texto')
...
def alter_dropPK(grandParent, parent, database, table):
    if database and table:
        ...
        if result == 0:
            ...
            messagebox.showinfo(title='Alter DropPK',
                                message='Llave primaria no existente')
    else:
        messagebox.showinfo(title='Alter DropPK',
                            message='No se llenaron los campos de texto')
...
def alter_table(grandParent, parent, actual, database, tableOld, tableNew, entry1):
    if database and tableOld and tableNew:
        ...
        if result == 0:
            ...
            messagebox.showinfo(title='Alter DropPK',
                                message=tableOld + ' no existente')
        elif result == 4:
            messagebox.showinfo(title='Alter DropPK',
                                message=tableNew+' nueva ya existe')
            entry1.delete(0, END)
    else:
        messagebox.showinfo(title='Alter DropPK',
                            message='No se llenaron los campos de texto')
...
def alter_addColumn_window(grandParent, parent, database, table):
    ...
    tkinter.Label(window, text='Add Column',
                  font='Helvetica 10 bold italic', width=20).place(x=25, y=70)
    Label(window, text='Default', font='Helvetica 10 bold italic',
          width=20).place(x=140, y=50)
...
def alter_addColumn(grandParent, parent, window, database, table, default, entry1):
    if database and table and default:
        ...
        if result == 0:
            messagebox.showinfo(title='Add Column',
                                message='Operacion exitosa')
            window.destroy()
            refresh_rows(grandParent, parent, database, table)
        elif result == 1:
...
def alter_dropColumn_window(grandParent, parent, database, table):
    ...
    tkinter.Label(window, text='Drop Column',
                  font='Helvetica 10 bold italic', width=20).place(x=25, y=70)
    Label(window, text='Column', font='Helvetica 10 bold italic',
          width=20).place(x=140, y=50)
    columns = Entry(window, width=20)
...
def alter_dropColumn(grandParent, parent, window, database, table, columnNumber, entry1):
    if database and table and columnNumber:
        if isNumber(columnNumber):
            result = Storage.alterDropColumn(
                database, table, int(columnNumber))
            if result == 0:
                messagebox.showinfo(title='Alter Drop Column',
                                    message='Operacion exitosa')
                window.destroy()
                refresh_rows(grandParent, parent, database, table)
            elif result == 1:
                ...
                messagebox.showinfo(
                    title='Alter Drop Column', message='Llave no puede eliminarse o la tabla se queda sin columnas')
            elif result == 5:
                messagebox.showinfo(title='Alter Drop Column',
                                    message='Columna fuera de limites')
                entry1.delete(0, END)
        else:
            messagebox.showinfo(title='Alter Drop Column',
                                message='No ingreso un numero de columnas')
            entry1.delete(0, END)
    else:
        messagebox.showinfo(title='Alter DropPK',
                            message='No se llenaron los campos de texto')
...
def insert_row_window(grandParent, parent, database, table):
    ...
    window.geometry('500x200+200+100')
    window.title('Insert')
    tkinter.Label(window, text='Insert',
                  font='Helvetica 10 bold italic', width=20).place(x=25, y=70)
    Label(window, text='List', font='Helvetica 10 bold italic',
          width=20).place(x=140, y=50)
...
def insert_row(grandParent, parent, window, database, table, register, entry1):
    if database and table and register:
        ...
        if result == 0:
            ...
            messagebox.showinfo(
                title='Insert', message='Error en la operacion')
            entry1.delete(0, END)
        elif result == 2:
            messagebox.showinfo(
                title='Insert', message='Base de datos no existente')
            entry1.delete(0, END)
        elif result == 3:
            ...
            messagebox.showinfo(
                title='Insert', message='Llave primaria duplicada')
            entry1.delete(0, END)
        elif result == 5:
            messagebox.showinfo(
                title='Insert', message='Columna fuera de limites')
            entry1.delete(0, END)
    else:
        messagebox.showinfo(title='Alter DropPK',
                            message='No se llenaron los campos de texto')
...
def load_csv_window(grandParent, parent, database, table):
    ...
    window.title('Load')
    tkinter.Label(window, text='Load CSV',
                  font='Helvetica 10 bold italic', width=20).place(x=25, y=70)
    Label(window, text='File  path', font='Helvetica 10 bold italic',
          width=20).place(x=140, y=50)
...
def load_csv(grandParent, parent, window, database, table, filepath, entry1):
    if filepath and database and table:
        ...
        messagebox.showinfo(
            title='Load CSV', message='No se llenaron los campos de texto')
        entry1.delete(0, END)
...
def extract_row_window(database, table):
    ...
    tkinter.Label(window, text='Extract Row',
                  font='Helvetica 10 bold italic', width=20).place(x=25, y=70)
    Label(window, text='List', font='Helvetica 10 bold italic',
          width=20).place(x=140, y=50)
    columns = Entry(window, width=20)
...
def extract_row(database, table, columns, entry1):
    if database and table and columns:
        ...
        messagebox.showinfo(title='Extract Row',
                            message='Resultado: '+str(result))
    else:
        messagebox.showinfo(title='Extract Row',
                            message='No se llenaron los campos de texto')
        entry1.delete(0, END)
...
def update_window(grandParent, parent, database, table):
    ...
    window.geometry('500x200+200+100')
    window.title('Update')
    Label(window, text='Update', font='Helvetica 10 bold italic',
          width=20).place(x=50, y=70)
    Label(window, text='Dictionary', font='Helvetica 10 bold italic',
          width=20).place(x=130, y=50)
    Label(window, text='Columns', font='Helvetica 10 bold italic',
          width=10).place(x=290, y=50)
    register = Entry(window, width=20)
    ...
    if database and table and register and columns:
        ...
        for x in separate_register:
            ...
            if not isNumber(data[0]):
                messagebox.showinfo(
                    title='Extract Row', message='No ingreso correctamente las claves')
                entry1.delete(0, END)
                return
            key = int(data[0])
        ...
        if result == 0:
            ...
            messagebox.showinfo(
                title='Update', message='Error en la operacion')
            entry1.delete(0, END)
            entry2.delete(0, END)
        elif result == 2:
            messagebox.showinfo(
                title='Update', message='Base de datos no existente')
            entry1.delete(0, END)
            entry2.delete(0, END)
        elif result == 3:
            ...
            messagebox.showinfo(
                title='Update', message='Llave primaria no existe')
            entry1.delete(0, END)
            entry2.delete(0, END)
    else:
        messagebox.showinfo(title='Extract Row',
...
def delete_window(grandParent, parent, database, table):
    ...
    window.geometry('500x200+200+100')
    window.title('Delete')
    tkinter.Label(window, text='Delete',
                  font='Helvetica 10 bold italic', width=20).place(x=25, y=70)
    Label(window, text='List', font='Helvetica 10 bold italic',
          width=20).place(x=140, y=50)
    columns = Entry(window, width=20)
...
def delete(grandParent, parent, window, database, table, columns, entry1):
    if database and table and columns:
        ...
        messagebox.showinfo(
            title='Update', message='No lleno todos los campos')
        entry1.delete(0, END)
...
def truncate_table(grandParent, parent, database, table):
    ...
    if pregunta:
        if database and table:
            ...
            if result == 0:
                messagebox.showinfo(
                    title='Update', message='Operacion exitosa')
                refresh_rows(grandParent, parent, database, table)
            elif result == 1:
                messagebox.showinfo(
                    title='Update', message='Error en la operacion')
                ...
                messagebox.showinfo(
                    title='Update', message='Tabla no existente')
        else:
            messagebox.showinfo(
                title='Update', message='No lleno todos los campos')
...
tkinter.Label(main_canvas, text='Tytus Database',
              font='Helvetica 30 bold italic', padx=10, pady=5).place(x=150, y=20)
tkinter.Button(main_canvas, text='Reportes', font='Helvetica 16 bold italic', width=20, height=2,
               borderwidth=5, fg='white', background='black', command=show_data_bases).place(x=175, y=150)
