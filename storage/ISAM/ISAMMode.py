    ...
    try:
        ...
        if databaseOld not in showDatabases():
            ...
            for i in showTables(databaseNew):
                os.rename('data/ISAMMode/tables/' + databaseOld + i + '.bin',
                          'data/ISAMMode/tables/' + databaseNew + i + '.bin')
            return 0
    except:
        return 1
...
def alterTable(database, tableOld, tableNew):
    ...
    try:
        if database not in showDatabases():
            ...
            os.rename('data/ISAMMode/tables/' + database + tableOld + '.bin',
                      'data/ISAMMode/tables/' + database + tableNew + '.bin')
            index = showDatabases().index(database)
