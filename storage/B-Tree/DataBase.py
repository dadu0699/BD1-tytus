            ...
            if self.searchDB(database):
                if self.searchTB(database, table):
                    ...
                    if self.identify(table):
                        try:
                            self.dicDB.get(database)[table] = [bt.arbolB(
                                self.grade), int(numberColumns), None]
                            return 0
                        except:
                            return 1
                    return 1
                    ...
                    if range:
                        if not self.dicDB[database][table][2]:
                            if self.verifyPk(database, table, columns):
                                ...
                                serializar.commit(
                                    self.dicDB[database][table][0], database+"-"+table+"-B")
                                return 0
                            return 4
                        return 4
                    return 5
                    ...
                    try:
                        ...
                        if not self.dicDB[database][table][2]:
                            while self.cont in self.dicDB[database][table][0].Keys():
                                self.cont += 1
                            self.dicDB[database][table][0].insertar(
                                [self.cont, register])
                            self.cont += 1
                            return 0
                        else:
